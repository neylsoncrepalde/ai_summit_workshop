    ...
    if prompt := st.chat_input("Enter your prompt here"):
        with st.spinner("Generating..."):
            ...
            if len(st.session_state.messages) > 4:
                complete_prompt = ''.join([str(i) for i in st.session_state.messages[-4:]]) + str({"role": "user", "content": prompt}) + "\nResponda em PT-BR."
            else:
                complete_prompt = ''.join([str(i) for i in st.session_state.messages]) + str({"role": "user", "content": prompt}) + "\nResponda em PT-BR."
